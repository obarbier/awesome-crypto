...
import (
	...
	domain2 "github.com/obarbier/awesome-crypto/pkg/user/domain"
...
var _ domain2.UserRepository = MongoRepository{}
...
func (m MongoRepository) Save(ctx context.Context, user *domain2.User) error {
	// Get the client
...
func (m MongoRepository) UpdateByID(ctx context.Context, id string, user *domain2.User) error {
	// Get the client
	client, err := m.Connection(ctx)
	if err != nil {
		return err
...
func (m MongoRepository) FindById(ctx context.Context, id string) (*domain2.User, error) {
	// Get the client
	client, err := m.Connection(ctx)
	if err != nil {
		return nil, err
	}
	var resp domain2.User
	// Run command
