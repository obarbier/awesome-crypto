...
func NewDefaultManageServer() *server.Server {
	...
	session.InitManager(
		session.SetStore(mongo.NewStore("mongodb://mongo:27017", "users", "session")),
	)
	manager := manage.NewDefaultManager()
	manager.SetAuthorizeCodeTokenCfg(manage.DefaultAuthorizeCodeTokenCfg)

	// token store
	manager.MustTokenStorage(store.NewMemoryTokenStore()) // TODO: implement with REDIS good practice
...
func handleLoginAuth(_ PropertiesHandler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		sessionStore, err := session.Start(r.Context(), w, r)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
